# Phase 3.6 dual runtime-controller R0.
# Source-only implementation. No parser, import, or execution evidence exists.

Set-StrictMode -Version Latest

$Script:HcamContractVersion = '1.0.0'
$Script:HcamAuthorizationDecision = 'D-P3.6-U3S-DUAL-CONTROLLER-R0-IMPLEMENTATION-AUTH'
...
<#
{"action_failure_reasons":{"bind_sources":"source_binding_failed","bind_utility_manifest_closure":"utility_manifest_closure_failed","classify_runtime_path":"runtime_path_classification_failed","classify_utility_manifest_path":"utility_manifest_path_failed","close_runtime_trust":"runtime_trust_close_failed","compare_cross_language_projection":"cross_language_projection_diverged","enforce_process_timeout":"process_timeout","hash_runtime":"runtime_hash_failed","initialize_runtime_trust":"runtime_trust_initialization_failed","parse_utility_manifest":"utility_manifest_parser_failed","read_runtime_metadata":"runtime_metadata_failed","reverify_runtime_identity":"runtime_identity_changed","start_validation_process":"process_start_failed","validate_authorization_binding":"authorization_binding_failed","validate_process_output_bounds":"process_output_bounds_failed","validate_request_contract":"request_contract_failed","validate_result_contract":"result_contract_invalid","verify_runtime_trust":"runtime_trust_verification_failed"},"action_order":["validate_request_contract","validate_authorization_binding","classify_runtime_path","read_runtime_metadata","hash_runtime","initialize_runtime_trust","verify_runtime_trust","close_runtime_trust","reverify_runtime_identity","classify_utility_manifest_path","parse_utility_manifest","bind_utility_manifest_closure","bind_sources","start_validation_process","enforce_process_timeout","validate_process_output_bounds","validate_result_contract","compare_cross_language_projection"],"authorization_fields":["decision_id","package_digest_sha256","attempt_index","attempt_state","expires_at_utc","authorization_window_state","source_hashes","machine_authority","automatic_retry"],"bound_fields":["per_action_timeout_ms","total_timeout_ms","max_stdout_bytes","max_stderr_bytes","max_result_bytes","max_closure_files","max_closure_bytes","max_processes","max_cleanup_actions"],"contract_version":"1.0.0","failure_reason_codes":["request_contract_failed","authorization_binding_failed","runtime_path_classification_failed","runtime_metadata_failed","runtime_hash_failed","runtime_trust_initialization_failed","runtime_trust_verification_failed","runtime_trust_close_failed","runtime_identity_changed","utility_manifest_path_failed","utility_manifest_parser_failed","utility_manifest_closure_failed","source_binding_failed","process_start_failed","process_timeout","process_output_bounds_failed","result_contract_invalid","cross_language_projection_diverged"],"input_fields":["stage_outcomes","elapsed_ms","stdout_bytes","stderr_bytes","result_bytes","closure_file_count","closure_total_bytes","retained_raw_bytes","cleanup_count","process_count","source_binding_match","cross_language_match"],"modes":["Policy","Preflight"],"prohibited_field_names":["acl","credentials","environment","exception","fixture_payload","hostname","ip_address","mac","manifest_text","manifest_tokens","native_status","parser_objects","password","personal_path","raw_exception","sddl","secret","serial","sid","stack","stderr","stdout","user","username"],"request_fields":["contract_version","mode","authorization_binding","declared_bounds","action_plan","input_bindings"],"result_fields":["contract_version","terminal","succeeded","reason_code","stage_projection","action_counts","retention_projection","gate_effect"],"source_hash_fields":["contract","vectors","powershell","python"]}
#>
...
function New-HcamTerminalProjection {
    ...
    $Succeeded = $ReasonCode -ceq 'policy_valid'
    return [ordered]@{
        contract_version = $Script:HcamContractVersion
        terminal = $true
        succeeded = $Succeeded
        reason_code = $ReasonCode
        stage_projection = [ordered]@{
            completed_actions = $CompletedActions
            failed_action = $FailedAction
            failed_stage = if ($Succeeded) { $null } else { $ReasonCode }
        }
...
function Invoke-HcamRuntimeController {
    param(
        [Parameter(Mandatory = $true)]
    ...
    if (-not $Inputs.source_binding_match) {
        return New-HcamActionFailureProjection -Action 'bind_sources' -ProcessCount $ProcessCount
    }
    ...
    return New-HcamTerminalProjection `
        ...
        -FailedAction $null `
        -CompletedActions $Script:HcamActionOrder.Count `
        -ProcessCount $ProcessCount `
        -CleanupCount $CleanupCount
}
