...
try {
    ...
    $script:RequiredUtilityModuleCommands = [string[]]@(
        'Compare-Object',
        'ConvertFrom-Json',
        'ConvertTo-Json',
        'Measure-Object',
    ...
    $script:RequiredCoreCommands = [string[]]@(
        'ForEach-Object',
        'Where-Object'
    )
    $script:RequiredUtilityCommands = [string[]]@(
        $script:RequiredUtilityModuleCommands + $script:RequiredCoreCommands
    )
    ...
    Import-Module -Name $script:UtilityManifestPath -Scope Local -NoClobber -Cmdlet $script:RequiredUtilityModuleCommands -Function @() -Alias @() -Variable @() -ErrorAction Stop
    ...
    foreach ($command in $resolvedUtilityCommands) {
        $expectedSource = if ($script:RequiredCoreCommands -ccontains $command.Name) {
            'Microsoft.PowerShell.Core'
        } else {
            $script:UtilityModuleName
        }
        if (
            $script:RequiredUtilityCommands -cnotcontains $command.Name -or
            $command.CommandType -cne 'Cmdlet' -or
            $command.Source -cne $expectedSource -or
            $command.ModuleName -cne $expectedSource -or
...
$script:ContractId = 'P36-QUARANTINE-GENERATED-VALIDATION-HARNESS-R5-1.5.0'
$script:RepositoryRoot = [System.IO.Path]::GetFullPath(
    [System.IO.Path]::Combine($PSScriptRoot, '..')
)
$script:HarnessPath = [System.IO.Path]::Combine(
    $PSScriptRoot,
    'phase36_quarantine_generated_validation_r5.ps1'
)
$script:RunnerPath = [System.IO.Path]::Combine(
    $PSScriptRoot,
    'phase36_quarantine_transaction_runner_r2.ps1'
)
$script:HandlerModulePath = [System.IO.Path]::Combine(
    $PSScriptRoot,
    'phase36_quarantine_machine_handlers_r1.psm1'
)
$script:WindowsAdapterParserPath = [System.IO.Path]::Combine(
    $PSScriptRoot,
    'phase36_quarantine_windows_storage_adapter_r1.psm1'
)
$script:VectorManifestPath = [System.IO.Path]::Combine(
    $script:RepositoryRoot,
    'contracts',
    'phase-3',
    'p3-6-quarantine-generated-powershell-validation-r5-vectors.json'
)
...
$script:HandlerCheckpoint = 'not_started'
...
$script:AcceptedHashes = [ordered]@{
    $script:RunnerPath = '8C3BB5A4CB473954251DDD4B24375AFDEE698FBA4F295A228B1E5898FA6F7189'
    $script:HandlerModulePath = '65D4756315B31CA6F8C7195B28173CD2AEC3977E968441434F66D05DBF67F16B'
    $script:WindowsAdapterParserPath = '96C8AFAFBA6C74728E2596753EB2DD198F25D526FDB7A2619FF8763C8E83CD51'
}
...
trap {
    ...
    $safeReasonCode = "$($safeFailureLayer)_failed"
    if ($safeFailureLayer -ceq 'handler') {
        $safeReasonCode = switch -CaseSensitive ($_.Exception.Message) {
            'P36_HANDLER_EXPORT_SET_INVALID' { 'handler_export_set_invalid' }
            'P36_HANDLER_VECTOR_ACTION_INVALID' { 'handler_vector_action_invalid' }
            'P36_HANDLER_PREFIX_FAILED' { 'handler_prefix_failed' }
            'P36_CLEANUP_VECTOR_MISSING' { 'handler_cleanup_vector_missing' }
            'P36_CROSS_VECTOR_SCENARIO_INVALID' { 'handler_cross_vector_invalid' }
            'P36_HANDLER_LAYER_TIMEOUT' { 'handler_timeout' }
            default {
                switch -CaseSensitive ($script:HandlerCheckpoint) {
                    'module_import' { 'handler_module_import_failed' }
                    'export_set' { 'handler_export_set_failed' }
                    'source_read' { 'handler_source_read_failed' }
                    'action_case' { 'handler_action_case_failed' }
                    'cross_case' { 'handler_cross_case_failed' }
                    default { 'handler_failed' }
                }
            }
        }
    }
...
function Invoke-P36HandlerLayer {
    ...
    $script:HandlerCheckpoint = 'module_import'
    Import-Module `
        ...
        -NoClobber `
        -ErrorAction Stop
    $script:HandlerCheckpoint = 'export_set'
    ...
    $script:HandlerCheckpoint = 'source_read'
    ...
    foreach ($vector in $Manifest.handler_vectors) {
        ...
        $matches = if ([string] $vector.kind -ceq 'action') {
            $script:HandlerCheckpoint = 'action_case'
            Invoke-P36GeneratedActionCase -Manifest $Manifest -Vector $vector
        } else {
            $script:HandlerCheckpoint = 'cross_case'
    ...
    $script:HandlerCheckpoint = 'complete'
    return [ordered]@{
