"""Machine-disabled reference for the Phase 3.6 controller R1 contract.

The module evaluates generated mappings only. It has no filesystem, registry,
...
STAGE_PROJECTION_FIELDS = ("completed_actions", "failed_action", "failed_stage")
...
def canonical_json(value: Any) -> str:
    """Return the canonical JSON representation used by both controllers."""
...
def contract_projection() -> dict[str, Any]:
    """Return constants that the PowerShell and Python sources must share."""
    ...
    return {
        ...
        "stage_projection_fields": list(STAGE_PROJECTION_FIELDS),
    }


def projections_equal(candidate: Any) -> bool:
    """Compare a static implementation projection to the canonical contract."""
...
def validate_request(request: Any) -> bool:
    """Validate the bounded, machine-independent request envelope."""
...
def _gate_effect(succeeded: bool) -> dict[str, Any]:
    return {
        ...
        "next_gate": "owner_source_implementation_acceptance_required"
        if succeeded
        else "blocked",
    }
...
def _result_envelope(
    ...
    completed_actions = stage_projection["completed_actions"]
    return {
        "contract_version": CONTRACT_VERSION,
        "terminal": True,
        "succeeded": succeeded,
        ...
        "gate_effect": _gate_effect(succeeded),
    }


def _policy_valid_result(process_count: int = 0, cleanup_count: int = 0) -> dict[str, Any]:
    stage_projection = {
        "completed_actions": len(ACTION_ORDER),
        "failed_action": None,
        "failed_stage": None,
    }
    return _result_envelope(
        "policy_valid", True, stage_projection, process_count, cleanup_count
    )
...
def _failure_result(
    ...
    reason_code = ACTION_FAILURE_REASONS[action]
    stage_projection = {
        "completed_actions": ACTION_ORDER.index(action),
        "failed_action": action,
        "failed_stage": reason_code,
    }
    return _result_envelope(
        reason_code, False, stage_projection, process_count, cleanup_count
    )


def validate_stage_projection(
    value: Any,
    *,
    succeeded: bool,
    reason_code: str,
) -> bool:
    """Enforce literal-null success and exact nonempty failure semantics."""

    if not _exact_fields(value, STAGE_PROJECTION_FIELDS):
        return False
    completed = value["completed_actions"]
    if type(completed) is not int or completed < 0 or completed > len(ACTION_ORDER):
        return False
    if succeeded:
        return (
            reason_code == "policy_valid"
            and completed == len(ACTION_ORDER)
            and value["failed_action"] is None
            and value["failed_stage"] is None
        )
    if reason_code not in FAILURE_REASONS:
        return False
    action = value["failed_action"]
    if not isinstance(action, str) or not action or action not in ACTION_FAILURE_REASONS:
        return False
    return (
        ACTION_FAILURE_REASONS[action] == reason_code
        and value["failed_stage"] == reason_code
        and completed == ACTION_ORDER.index(action)
    )


def evaluate_request(request: Any) -> dict[str, Any]:
    """Evaluate one generated request without touching machine state."""

    if not validate_request(request):
        return _failure_result("validate_request_contract")
    ...
    return _policy_valid_result(inputs["process_count"], inputs["cleanup_count"])


def validate_result(result: Any) -> bool:
    """Validate a terminal result and reject null/empty-string ambiguity."""
    ...
    if result["reason_code"] not in ("policy_valid", *FAILURE_REASONS):
        return False
    nested_fields = (
        ("stage_projection", STAGE_PROJECTION_FIELDS),
        ("action_counts", ACTION_COUNT_FIELDS),
        ("retention_projection", RETENTION_FIELDS),
        ("gate_effect", GATE_EFFECT_FIELDS),
    )
    if not all(_exact_fields(result[name], fields) for name, fields in nested_fields):
        return False
    succeeded = result["succeeded"]
    reason_code = result["reason_code"]
    if succeeded != (reason_code == "policy_valid"):
        return False
    if not validate_stage_projection(
        result["stage_projection"], succeeded=succeeded, reason_code=reason_code
    ):
        return False
    counts = result["action_counts"]
    if not all(_is_nonnegative_integer(counts[name]) for name in ACTION_COUNT_FIELDS):
        return False
    if counts != {
        "planned": len(ACTION_ORDER),
        "completed": result["stage_projection"]["completed_actions"],
        "attempts": 1,
        "retries": 0,
        "processes": counts["processes"],
    }:
        return False
    retention = result["retention_projection"]
    if not _is_nonnegative_integer(retention["cleanup_actions"]):
        return False
    if type(retention["sanitized_result_only"]) is not bool:
        return False
    if retention != {
        "raw_material_retained_bytes": 0,
        "sanitized_result_only": True,
        "cleanup_actions": retention["cleanup_actions"],
    }:
        return False
    gate_effect = result["gate_effect"]
    boolean_gates = GATE_EFFECT_FIELDS[:-1]
    if not all(type(gate_effect[name]) is bool for name in boolean_gates):
        return False
    return gate_effect == _gate_effect(succeeded)
