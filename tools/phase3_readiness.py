...
def check_contract_snapshots() -> CheckResult:
    ...
    merged_gis_head = (
        heads == ["0012_merge_camera_gis"]
        and isinstance(lineage, list)
        and {"0011_geometry_events", "0008_postgis_gis"}.issubset(lineage)
    )
    if (
        not isinstance(heads, list)
        or tuple(heads) not in allowed_heads
        and not merged_gis_head
    ):
        failures.append(
            "database snapshot is not at an approved P3.0 through P3.4 revision"
        )
