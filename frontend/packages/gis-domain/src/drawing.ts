...
/**
 * H-CAM generated area-of-interest shape model.
 *
...
export type DrawGeometry =
  { type: "Polygon"; coordinates: number[][][] } | { type: "LineString"; coordinates: number[][] };
...
export function nextColor(existing: { color: string }[]): string {
  return PALETTE[existing.length % PALETTE.length] ?? "#00E5FF";
}
...
export function minPoints(mode: DrawMode): number {
  switch (mode) {
    ...
    case "circle":
      return 2; // centre + a point on the rim
  }
}
...
export function measure(mode: DrawMode, points: number[][]): DrawProgress {
  ...
  if (mode === "circle") {
    const radiusKm =
      points.length >= 2 ? haversine(points[0] as LngLat, points[points.length - 1] as LngLat) : 0;
...
export function toShape(
  ...
  const geometry: DrawGeometry = isLine
...
export function queryRing(shape: DrawnShape): number[][] | null {
  return shape.geojson.geometry.type === "Polygon"
    ? (shape.geojson.geometry.coordinates[0] ?? null)
    : null;
}
  ...
  const meta =
    state.mode === "circle"
      ? {
          ...
          radiusKm: haversine(
            state.points[0] as LngLat,
            state.points[state.points.length - 1] as LngLat,
          ),
        }
      : undefined;
  return { kind: state.mode, coords, ...(meta ? { meta } : {}) };
}
  ...
  switch (action.type) {
    case "click": {
      ...
      if ((state.mode === "rectangle" || state.mode === "circle") && points.length >= 2) {
        const result = complete(next);
        return result ? { state: initialDrawState(state.mode), result } : { state: next };
      }
      return { state: next };
    }
