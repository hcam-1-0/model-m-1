/**
 * H-CAM geodesic primitives for generated local planning geometry.
 *
...
export function polygonArea(ring: number[][]): number {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const lat1 = toRad(current[1] ?? 0);
    const lat2 = toRad(next[1] ?? 0);
    const dLng = toRad((next[0] ?? 0) - (current[0] ?? 0));
